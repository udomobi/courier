...
func (h *handler) SendMsg(ctx context.Context, msg courier.Msg) (courier.MsgStatus, error) {
	...
	if len(msg.Attachments()) > 0 {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "unable to decode template: %s for channel: %s", string(msg.Text()), msg.Channel().UUID())
		}
		if templating != nil {
...
func (h *handler) getTemplate(msg courier.Msg) (*MsgTemplating, error) {
	mdJSON := msg.Text()
	if len(mdJSON) == 0 || !strings.HasPrefix(mdJSON, prefixMetadata) {
		return nil, nil
	}
	mdJSON = mdJSON[len(prefixMetadata):]

	metadata := &TemplateMetadata{}
	err := json.Unmarshal([]byte(mdJSON), metadata)
	if err != nil {
		return nil, err
	}
